...
export default function PendingActionModal({ gameId, sessionToken, userId, pendingAction, players, onResolved, activePlayerHasHighStockPrices = false }) {
  ...
  // Fire Destroys Haystack reconciliation popup
  if (pendingAction.action_type === 'tucker_bag_result' && data.effect_code === 'FIRE_DAMAGE') {
    return (
      <div style={modalStyle}>
        <h2 style={{ margin: '0 0 0.5rem', color: '#d32f2f' }}>
  ...
  // Worm Infestation reconciliation popup
  if (pendingAction.action_type === 'tucker_bag_result' && data.effect_code === 'WORM_INFESTATION') {
    return (
      <div style={modalStyle}>
  ...
  // Blowfly Wave announcement
  if (pendingAction.action_type === 'tucker_bag_result' && data.effect_code === 'BLOWFLY_WAVE') {
    return (
      <div style={modalStyle}>
        <h2 style={{ margin: '0 0 0.5rem', color: '#d32f2f' }}>
  ...
  // Grass Fire — PROTECTED case only (centred modal). Unprotected case has
  // stock_card_used set and is rendered as the board-anchored overlay.
  if (pendingAction.action_type === 'tucker_bag_result' && data.effect_code === 'GRASS_FIRE' && data.protected) {
    return (
      <div style={modalStyle}>
        <h2 style={{ margin: '0 0 0.5rem', color: '#d32f2f' }}>
